...
containing the numbers that occur in elements of strArr in sorted
...
const findIntersection = (arr) => {
  const firstArr = arr[0].split(",").map((item) => item.trim());
  const secondArr = arr[1].split(",").map((item) => item.trim());
